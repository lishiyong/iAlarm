...
//lishiyong 2012-3-16 添加
- (id)initWithAppId:(NSString *)app_id andSessionDelegate:(id<FBSessionDelegate>)sessionDelegate{
    self = [super init];
    if (self) {
        [_appId release];
        _appId = [app_id copy];
        ...
        if ([_sessionDelegate respondsToSelector:@selector(cachedFacebookExpirationDate)]) {
            _expirationDate = [[_sessionDelegate cachedFacebookExpirationDate] copy]; //原来就是声明成copy的，虽然没道理，但还是先不按copy做
        }
        if ([_sessionDelegate respondsToSelector:@selector(cachedFacebookAccessToken)]) {
            _accessToken = [[_sessionDelegate cachedFacebookAccessToken] copy]; //string类型的属性声明成copy是对的
        }
...
- (void)dealloc {
	...
	[_fbDialog dismissWithSuccess:NO animated:NO];
	[_request.connection cancel];//否则不能释放_request
...
- (FBRequest*)openUrl:(NSString *)url
 ...
 //NSLog(@"openUrl-Facebook.request.retaincount = %d",[_request retainCount]);
...
- (void)fbDialogLogin:(NSString *)token expirationDate:(NSDate *)expirationDate {
  ...
  if ([self.sessionDelegate respondsToSelector:@selector(fbDidLogin)]) {
    ...
    //lishiyong 2012-3-16 添加
    if ([self.sessionDelegate respondsToSelector:@selector(storeAuthData:expiresAt:username:)]) {
        [_sessionDelegate storeAuthData:self.accessToken expiresAt:self.expirationDate username:nil];
    }
...
//lishiyong 2012-3-17 添加
- (NSString*)username{
    if ([self.sessionDelegate respondsToSelector:@selector(cachedFacebookUsername)]) {
        return [_sessionDelegate cachedFacebookUsername];
    }
    return nil;
}
